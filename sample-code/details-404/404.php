    ...
    {% spaceless %}
        ...
        <div id="IDX-detailsWrapper" class="IDX-pageContainer" data-collapse-details="{{ collapseMobileFirstDetailsFields }}">
            ...
            <h1>404</h1>
             ...
             <h2>It can be tough to catch exactly what you are looking for online</h2>
             <div id="kitty">
             <iframe src="https://giphy.com/embed/f08cafueoCoRW" width="100%" height="100%" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
               </div>
               <div id="search">
               <h2>Let's Search for Some Nearby Properties. Just Choose Below</h2>
                <select id="nearBy" onChange="searchAgain(this)">
                <option value="doNothing">Continue Your Search</option>
                <option id="nearProp" value="" >Properties Near The One I Was Searching For</option>
                <option id="nearMe" value="nearMe">Properties Near Me</option>
                </select>
                </div>
                <div id="orContact">
                <h2>Or contact us by filling out the form below!</h2>
                </div>
                
                <script>
            	var rad = "1.000";
                
                function searchAgain(v) {
                  ...
                  if (v.value == 'nearMe') {
                            if (navigator.geolocation) {
                                ...
                                function showPosition(position) {
                                    ...
                                    var myPath = '/idx/results/listings?pgon='+myLat+'+'+myLng+'&radius=1.0427&layerType=circle&clat='+myLat+'&clng='+myLng+'&zoom=13&idxID=a001&pt=1&srt=prd';
                                    window.location.href = here + myPath;
                                }
                  ...
                  else if(v.value == 'doNothing') {
                  	document.querySelector('#nearProp').value = 'doNothing';
                  }
                  else {
              	...
              	//location path and prep for google api
                ...
                axios.get('https://maps.googleapis.com/maps/api/geocode/json?address='+address+'+&key=AIzaSyCWiSKCiaA-Wu95I7Jbwmutu5IXBLwL8hM')
  					.then(function (response) {
                        ...
                        var lat = latLng.lat;
                        var lng = latLng.lng;
                		var radSearch = '/idx/results/listings?pgon='+lat+'+'+lng+'&radius=1.0427&layerType=circle&clat='+lat+'&clng='+lng+'&zoom=13&idxID=a001&pt=1&srt=prd';
                        document.querySelector('#nearProp').value = window.location.origin + radSearch;
                            ...
                            {% if idxID == 'a999' or mlsRules.disableShareThis == 'n' %}
                                ...
                                <div id="IDX-sharethis" class="IDX-row-content IDX-center">
                                    <span
                                        ...
                                        st_url='{{ propertyLink }}'
                                        st_title='I just found a property and wanted to share it with you!'
                                        st_summary='Above is the link to the property that I found. What do you think?'
                                        st_image='{{ imageData|first.url }}'
                                    ></span>
                                    <span
